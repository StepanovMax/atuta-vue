const testObjects = [
  {
    parentID: 1,
    status: 2,
    id: 873589,
    object: {
      slug: 'app',
      label: 'Квартира',
    },
    type: {
      label: 'Вторичка',
      slug: 'secondaryUsing',
    },
    view: {
      slug: 'brick',
      label: 'Кирпичный',
    },
    deal: {
      slug: 'rent',
      label: 'Аренда',
    },
    address: 'Ростовская область, Таганрог, улица Шевченко, 15',
    coords: [
      47.20133956793496,
      38.93260869854734
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    roomsCount: {
      label: '9+',
      slug: '9+',
    },
    area: {
      label: '40',
      slug: '40',
    },
    areaKitchen: {
      label: '10',
      slug: '10',
    },
    areaLiving: {
      label: '20',
      slug: '20',
    },
    year: {
      label: '1970',
      slug: '1970',
    },
    floor: {
      label: '3',
      slug: '3',
    },
    floorAll: {
      label: '5',
      slug: '5',
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    distance: {
      value: 15,
    },
    urlPreview: '/src/images/objects/8993850262.jpg',
    phoneNumber: '79612701887',
    tarif: {
      label: 'Premium',
      slug: 'premium',
    },
    price: '2100000',
    date: '1596317207',
    user: {
      type: {
        label: 'Агентство',
        slug: 'agency'
      },
      name: '"Агентство недвижимости №1"',
      contact: {
        label: 'Миргородский Иван Семёнович',
        slug: 'MirgorodskiyIvanSemenovich',
        phoneNumber: '79188981612',
      },
    },
    description: 'с.Приморка. Продается дом на двух частках! Стройвариант, свет, вода в доме. Скважина во дворе, газ помеже. Площадь участков 18 соток, дом 160 кв.м. на первом этаже зал, кухня, санузел.Второй этаж свободная планировка. Дом расположен в природоохранной зоне. . Курортное село на берегу Таганрогского залива .В селе есть школа(автобус), садик, клуб, рестораны, базы отдыха! До Таганрога 15 км, до Ростова 45 км. Ходит электричка, автобус. торг уместен. Если вы не дозвонились, пишите в Вацап. Я перезвоню!!!',
    title: "9+ ком., 40 м², этаж 3/5, 2 100 000₽",
    metaTitle: "Продам квартиру 3 000 000₽ 3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [],
  },
  {
    parentID: 1,
    status: 2,
    id: 873592,
    object: {
      slug: 'house',
      label: 'Дом',
    },
    view: {
      slug: 'cottage',
      label: 'Коттедж',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Чехова, 218',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    roomsCount: {
      label: '9+',
      slug: '9+',
    },
    area: {
      label: '60',
      slug: '60',
    },
    areaHouse: {
      label: '32',
      slug: '32',
    },
    areaSector: {
      label: '4',
      slug: '4',
    },
    year: {
      label: '1972',
      slug: '1972',
    },
    wall: {
      slug: 'brick',
      label: 'Кирпич',
    },
    floor: {
      label: '3',
      slug: '3',
    },
    distance: {
      value: 15,
    },
    photoGallery: [],
    urlPreview: '',
    phoneNumber: '79612701887',
    tarif: {
      label: 'Vip',
      slug: 'vip',
    },
    price: '1126000',
    date: '1596317207',
    user: {
      type: {
        label: 'Частное лицо',
        slug: 'person'
      },
      name: 'Максим',
      contact: {
        label: 'Степанов Максим Евгеньевич',
        slug: 'StepanovMaximEvgeievich',
        phoneNumber: '79612701887',
      },
    },
    description: 'Продам домовладение на 18 сот. ВАРЕНОВКА- пригород Таганрога! Два чисто кирпичных дома на участке (58+27м кв. + летняя кухня 20 м.) Газ, свет, вода, дом благоустроен, газ.котел, отопление на два дома, подвал,посажен молодой сад,виноград. Дом и земля оформлены в собственность, участок чистый.Пригород Таганрога 9 км. Вареновка. ( школа,детсады, храм, магазины и пр.) Хорошее транспортное сообщение с Таганрогом и Ростовом-на-Дону (электрички и автобусы,такси) Улица- Асфальт Пишите на АВИТО,подробности по тел. Хозяин.',
    metaTitle: "Продам дом 1 126 000₽ [3комн., 100м², 4сот.]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [
      {
        dialogID: 2,
        clientID: 1,
      },
    ],
  },
  {
    parentID: 1,
    status: 3,
    id: 873591,
    object: {
      slug: 'room',
      label: 'Комната',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Чехова, 218',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    view: {
      slug: 'block',
      label: 'Блочный',
    },
    roomsCount: {
      label: '9+',
      slug: '9+',
    },
    area: {
      label: '60',
      slug: '60',
    },
    year: {
      label: '1982',
      slug: '1982',
    },
    floor: {
      label: '3',
      slug: '3',
    },
    floorAll: {
      label: '5',
      slug: '5',
    },
    distance: {
      value: 15,
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    urlPreview: '/src/images/objects/8993850241.jpg',
    phoneNumber: '79612701887',
    tarif: {
      label: 'Up',
      slug: 'up',
    },
    price: '26000',
    date: '1596317207',
    user: {
      type: {
        label: 'Застройщик',
        slug: 'builder'
      },
      name: 'Строительная бригада',
      contact: {
        label: 'Петров Сергей Владимирович',
        slug: 'PetrovSergeyVladimirovich',
        phoneNumber: '79068975413',
      },
    },
    metaTitle: "Продам комнату 1 100 000₽ [3комн., 100м², этаж 3/13]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [
      {
        dialogID: 1,
        clientID: 1,
      },
      {
        dialogID: 2,
        clientID: 2,
      },
    ],
  },
  {
    parentID: 1,
    status: 4,
    id: 873599,
    object: {
      slug: 'garage',
      label: 'Гараж',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Чехова, 218',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    type: {
      label: 'Гараж',
      slug: 'garage',
    },
    garageType: {
      label: 'Железобетонный',
      labelShort: 'Ж/б',
      slug: 'reinforcedConcrete',
    },
    year: {
      label: '1999',
      slug: '1999',
    },
    area: {
      label: '30',
      slug: '30',
    },
    security: {
      label: 'Да',
      slug: 'yes',
    },
    distance: {
      value: 15,
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    urlPreview: '/src/images/objects/8993850241.jpg',
    phoneNumber: '79612701887',
    tarif: null,
    price: '320000',
    date: '1596317207',
    user: {
      type: {
        label: 'Агент',
        slug: 'agent'
      },
      name: 'Степанов Максим',
      contact: {
        label: 'Степанов Максим Евгеньевич',
        slug: 'StepanovMaximEvgeievich',
        phoneNumber: '79612701887',
      },
    },
    metaTitle: "Продам гараж 580 000₽ [ж/б, 100м²]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [],
  },
  {
    parentID: 1,
    status: 3,
    id: 873593,
    object: {
      slug: 'sector',
      label: 'Участок',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Чехова, 218',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    type: {
      slug: 'settlements',
      label: 'Поселений (ИЖС)',
      labelShort: 'ИЖС',
    },
    year: {
      label: '2001',
      slug: '2001',
    },
    area: {
      label: '30',
      slug: '30',
    },
    facade: {
      value: 13,
    },
    availabilityOfBuildings: {
      label: 'Да',
      slug: 'yes',
    },
    distance: {
      value: 15,
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    urlPreview: '/src/images/objects/8993850241.jpg',
    phoneNumber: '79612701887',
    tarif: null,
    price: '1100000',
    date: '1596317207',
    user: {
      type: {
        label: 'Агентство',
        slug: 'agency'
      },
      name: '"Тандем" Риелторская компания',
      contact: {
        label: 'Миргородский Иван Семёнович',
        slug: 'MirgorodskiyIvanSemenovich',
        phoneNumber: '79188981612',
      },
    },
    metaTitle: "Продам участок 1 230 000₽ [ИЖС, 20сот.]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [
      {
        dialogID: 1,
        clientID: 1,
      },
      {
        dialogID: 2,
        clientID: 2,
      },
    ],
  },
  {
    parentID: 1,
    status: 2,
    id: 873594,
    object: {
      slug: 'commercial',
      label: 'Коммерция',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Чехова, 218',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    type: {
      slug: 'sector',
      label: 'Участок',
    },
    year: {
      label: '2001',
      slug: '2001',
    },
    area: {
      label: '30',
      slug: '30',
    },
    tenant: {
      label: 'Да',
      slug: 'yes',
    },
    class: {
      label: 'C',
      slug: 'c',
    },
    distance: {
      value: 15,
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    urlPreview: '/src/images/objects/8993850241.jpg',
    phoneNumber: '79612701887',
    tarif: null,
    price: '2300000',
    date: '1596317207',
    user: {
      type: {
        label: 'Агентство',
        slug: 'agency'
      },
      name: '"Абсолют"',
      contact: {
        label: 'Васин Иван Петрович',
        slug: 'VasinIvanPetrovich',
        phoneNumber: '79092701901',
      },
    },
    metaTitle: "Продам участок под коммерцию 1 230 000₽ [30сот., Арендатор: да]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [
      {
        dialogID: 1,
        clientID: 1,
      },
    ],
  },
  {
    parentID: 1,
    status: 4,
    id: 873595,
    object: {
      slug: 'commercial',
      label: 'Коммерция',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Чехова, 218',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Северный',
      slug: 'severniy',
    },
    type: {
      "label": "Участок",
      "slug": "sector",
    },
    floor: {
      label: '4',
      slug: '4',
    },
    floorAll: {
      label: '6',
      slug: '6',
    },
    year: {
      label: '2001',
      slug: '2001',
    },
    area: {
      label: '30',
      slug: '30',
    },
    tenant: {
      label: 'Да',
      slug: 'yes',
    },
    distance: {
      value: 15,
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    urlPreview: '/src/images/objects/8993850241.jpg',
    phoneNumber: '79612701887',
    tarif: null,
    price: '2300000',
    date: '1596317207',
    user: {
      type: {
        label: 'Агентство',
        slug: 'agency'
      },
      name: '"Тандем"',
      contact: {
        label: 'Бродский Иосиф Александрович',
        slug: 'BrodskiyIosifAlexandrovich',
        phoneNumber: '79041941002',
      },
    },
    metaTitle: "Продам участок под коммерцию 1 230 000₽ [30сот., Арендатор: да]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [
      {
        dialogID: 1,
        clientID: 1,
      },
      {
        dialogID: 2,
        clientID: 2,
      },
    ],
  },
  {
    parentID: 1,
    status: 1,
    id: 873596,
    object: {
      slug: 'commercial',
      label: 'Коммерция',
    },
    deal: {
      slug: 'buy',
      label: 'Купить',
    },
    address: 'Ростовская область, Таганрог, ул. Транспортная, 12',
    coords: [
      47.22064,
      38.914713
    ],
    district: {
      label: 'Южный',
      slug: 'yuzhniy',
    },
    type: {
      "slug": "stock",
      "label": "Склад",
    },
    floor: {
      label: '4',
      slug: '4',
    },
    floorAll: {
      label: '6',
      slug: '6',
    },
    year: {
      label: '2008',
      slug: '2008',
    },
    area: {
      label: '30',
      slug: '30',
    },
    tenant: {
      label: 'Нет',
      slug: 'no',
    },
    distance: {
      value: 4,
    },
    photoGallery: [
      '/src/images/objects/8993850241.jpg',
      '/src/images/objects/8993850262.jpg',
      '/src/images/objects/8993850286.jpg',
    ],
    urlPreview: '/src/images/objects/8993850241.jpg',
    phoneNumber: '79612701887',
    tarif: null,
    price: '2300000',
    date: '1596317207',
    user: {
      type: {
        label: 'Частное лицо',
        slug: 'person'
      },
      name: 'Тавадян Артур',
      contact: {
        label: 'Тавадян Артур Армоисович',
        slug: 'TavadanArturArmoisovich',
        phoneNumber: '79289634879',
      }
    },
    metaTitle: "Продам участок под коммерцию 1 230 000₽ [30сот., Арендатор: да]",
    title: "3 ком., 100 м², этаж 3/13",
    viewings: {
      value: 688
    },
    onlineShow: {
      slug: 'yes',
      label: 'Да',
    },
    dialogsList: [
      {
        dialogID: 1,
        clientID: 1,
      },
      {
        dialogID: 2,
        clientID: 2,
      },
    ],
  },
];

export default testObjects;