<template>
  <svg
    width="20"
    height="18"
    viewBox="0 0 20 18"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    class="icon"
  >
    <rect
      y="14"
      width="20"
      height="2"
      class="icon__path"
    />
    <rect
      y="8"
      width="20"
      height="2"
      class="icon__path"
    />
    <rect
      y="2"
      width="20"
      height="2"
      class="icon__path"
    />
    <path
      d="M8 3C8 4.65685 6.65685 6 5 6C3.34315 6 2 4.65685 2 3C2 1.34315 3.34315 0 5 0C6.65685 0 8 1.34315 8 3Z"
      class="icon__path"
    />
    <path
      d="M7 3C7 4.10457 6.10457 5 5 5C3.89543 5 3 4.10457 3 3C3 1.89543 3.89543 1 5 1C6.10457 1 7 1.89543 7 3Z"
      fill="white"
    />
    <path
      d="M18 9C18 10.6569 16.6569 12 15 12C13.3431 12 12 10.6569 12 9C12 7.34315 13.3431 6 15 6C16.6569 6 18 7.34315 18 9Z"
      class="icon__path"
    />
    <path
      d="M17 9C17 10.1046 16.1046 11 15 11C13.8954 11 13 10.1046 13 9C13 7.89543 13.8954 7 15 7C16.1046 7 17 7.89543 17 9Z"
      fill="white"
    />
    <path
      d="M8 15C8 16.6569 6.65685 18 5 18C3.34315 18 2 16.6569 2 15C2 13.3431 3.34315 12 5 12C6.65685 12 8 13.3431 8 15Z"
      class="icon__path"
    />
    <path
      d="M7 15C7 16.1046 6.10457 17 5 17C3.89543 17 3 16.1046 3 15C3 13.8954 3.89543 13 5 13C6.10457 13 7 13.8954 7 15Z"
      fill="white"
    />
  </svg>
</template>

<script>
export default {
  name: 'iconSettings'
};
</script>